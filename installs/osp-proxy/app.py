...
def adaptive(endpoint, channelLocation):
    ...
    if hasattr(config, "forceDestination"):
        ...
        if rdis.exists(channelLocation) == False:
            ...
            return redirect(
                "/" + upstream + "/" + endpoint + "/" + channelLocation + "m3u8"
            )
        else:
            return redirect(
                "/"
                + str(rdis.get(channelLocation).decode("utf-8"))
                + "/"
                + endpoint
                + "/"
                + channelLocation
...
def home(endpoint, channelLocation, file):
    ...
    if fileExt == "key":
        if "X-Token-Session" in request.headers:
            ...
            authCheck = requests.post(
                config.ospCoreAPI + "/apiv1/rtmp/playbackauth",
                data={
                    "username": hlsToken[0],
                    "addr": clientIp,
            ...
            if (
                returnDataJson["results"] is False
                or returnDataJson["results"] == "False"
                or returnDataJson["results"] == "false"
            ):
                abort(403)
        else:
    ...
    if hasattr(config, "forceDestination"):
        ...
        if rdis.exists(channelParsed) == False:
            ...
            return redirect(
                "/" + upstream + "/" + endpoint + "/" + channelLocation + "/" + file
            )
        else:
            return redirect(
                "/"
                + str(rdis.get(channelParsed).decode("utf-8"))
                + "/"
                + endpoint
                + "/"
                + channelLocation
