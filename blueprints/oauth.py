...
from app import oAuthProviderClients
...
def oAuthLogin(provider):
    ...
    if sysSettings is not None:
        ...
        return oAuthProviderClients[provider].authorize_redirect(redirect_url)
    else:
...
def oAuthAuthorize(provider):
    ...
    if oAuthProviderQuery is not None:
        token = oAuthProviderClients[provider].authorize_access_token()
        userData = oAuthProviderClients[provider].get(oAuthProviderQuery.profile_endpoint)
