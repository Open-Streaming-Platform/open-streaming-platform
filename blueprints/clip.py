    ...
    if clipQuery is not None:
        ...
        if recordedVid is not None:
            ...
            if isEmbedded is None or isEmbedded == "False":
                ...
                randomClips = RecordedVideo.Clips.query.filter(RecordedVideo.Clips.id != clipQuery.id).filter(RecordedVideo.Clips.published == True).order_by(func.random()).limit(16)
