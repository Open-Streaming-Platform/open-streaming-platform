...
sudo openssl ec -in /opt/osp/vapid/vapid_private.pem -outform DER|tail -c +8|head -c 32|base64|tr -d '=' |tr -d '\n' |tr '/+' '_-' > /opt/osp/vapid/private_key.txt
sudo openssl ec -in /opt/osp/vapid/vapid_private.pem -pubout -outform DER|tail -c 65|base64|tr '/+' '_-'|tr -d '\n' > /opt/osp/vapid/public_key.txt