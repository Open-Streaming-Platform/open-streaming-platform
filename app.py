...
def check_isUserValidChannelViewer(userID,channelID):
...
def view_page(loc):
    ...
    if requestedChannel is not None:
        ...
        if sysSettings.adaptiveStreaming is True:
            ...
            streamURL = 'rtmp://' + sysSettings.siteAddress + ":1935/stream-data/" + requestedChannel.channelLoc +"?username=" + username + "&hash=" + secureHash
        ...
        if isEmbedded == None or isEmbedded == "False":
            ...
            return render_template('themes/' + sysSettings.systemTheme + '/channelplayer.html', stream=streamData, streamURL=streamURL, topics=topicList, randomRecorded=randomRecorded, channel=requestedChannel, secureHash=secureHash)
        else:
            ...
            if requestedUser is not None:
                ...
                if isValid is True:
                    if streamQuery.owningUser == requestedUser.id:
                        ...
                        if check_isUserValidChannelViewer(requestedUser.id,streamQuery.id):
                            return 'OK'
