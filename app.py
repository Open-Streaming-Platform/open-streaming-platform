...
def main_page():
    ...
    if firstRunCheck is False:
        ...
        return render_template('index.html', streamList=activeStreams, randomRecorded=randomRecorded)
...
def channel_view_page(chanID):
    ...
    return render_template('videoListView.html', channelData=channelData, openStreams=openStreams, recordedVids=recordedVids, title="Channels - Videos")
...
def topic_view_page(topicID):
    ...
    return render_template('videoListView.html', openStreams=streamsQuery, recordedVids=recordedVideoQuery, title="Topics - Videos")
...
def streamers_view_page(userID):
    ...
    return render_template('videoListView.html', openStreams=streams, recordedVids=recordedVideoQuery, title=userName + " - Videos")
    ...
    if streamData is not None:
        ...
        if chatOnly == "True" or chatOnly == "true":
            if streamData.channel.chatEnabled == True:
                return render_template('chatpopout.html', stream=streamData, streamURL=streamURL)
            else:
        ...
        if isEmbedded == None or isEmbedded == "False":
            ...
            if isAutoPlay == None:
                ...
                isAutoPlay = False
            return render_template('player_embed.html', stream=streamData, streamURL=streamURL, topics=topicList, isAutoPlay=isAutoPlay)
...
def view_vid_page(videoID):
    ...
    if isEmbedded == None or isEmbedded == "False":
        ...
        if isAutoPlay == None:
            ...
            isAutoPlay = False
        return render_template('vidplayer_embed.html', video=recordedVid, streamURL=streamURL, topics=topicList, isAutoPlay=isAutoPlay)
...
def admin_page():
    if request.method == 'GET':
            ...
            if action == "delete":
                ...
                if setting == "channel":
                    ...
                    if userQuery != None:
                        ...
                        #TODO Add function to Delete All owned Channels and Recorded Videos
        ...
        if settingType == "system":
            ...
            app.config.update(MAIL_USE_SSL=sysSettings.smtpSSL)
            app.config.update(MAIL_USE_TLS=sysSettings.smtpTLS)
...
def settings_apikeys_page():
    ...
    return render_template('apikeys.html',apikeys=apiKeyQuery)
...
def initialSetup():
    ...
    if firstRunCheck is False:
        ...
        if password1 == password2:
            ...
            serverSettings = settings.settings(serverName, serverAddress, smtpAddress, smtpPort, smtpTLS, smtpSSL, smtpUser, smtpPassword, smtpSendAs, registerSelect, recordSelect)
    ...
    if channelRequest is not None:
        ...
        if userQuery != None:
            if userQuery.has_role('Streamer'):
                ...
                print(returnMessage)
                externalIP = sysSettings.siteAddress
                try:
                    externalIP = socket.gethostbyname(sysSettings.siteAddress)
                except:
                    pass
                newStream = Stream.Stream(key, normalize_date(str(datetime.datetime.now())), int(channelRequest.id), channelRequest.topic)
                ...
                returnMessage = {'time': str(datetime.datetime.now()), 'status': 'Unauthorized User - Missing Streamer Role', 'key': str(key), 'ipAddress': str(ipaddress)}
                print(returnMessage)
                return abort(400)
        else:
            returnMessage = {'time': str(datetime.datetime.now()), 'status': 'Unauthorized User - No Such User', 'key': str(key), 'ipAddress': str(ipaddress)}
            print(returnMessage)
            return abort(400)
    else:
